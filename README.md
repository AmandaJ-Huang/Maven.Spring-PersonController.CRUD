...
- `POST /people` - create a new io.zipcoder.crudapp.person
  - Response: `201 Created`
- `GET /people` - get the list of all people
  - Response: `200 OK` 
- `GET /people/{id}` - Get the io.zipcoder.crudapp.person with id number `{id}`
  - Response: `200 OK` if found, else `404 Not Found`
- `PUT /people/{id}` - Update the io.zipcoder.crudapp.person with id number `{id}`
  - Response: `200 OK` if updated, `201 Created` if a new entity was created
- `DELETE /people/{id}` - delete the io.zipcoder.crudapp.person with id number `{id}`
